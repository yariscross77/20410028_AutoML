...
def main():
    ...
    if choice == 'グラフ表示':
        ...
        if 'df' in st.session_state:
            ...
            if choice == activities[0] :
                ...
                st_display_graph(df,x_col="月給(ドル)")
    ...
    if choice == '学習と検証':
        ...
        if 'df' in st.session_state:
            ...
            # サイドメニューの設定
            activities = ["決定木","ランダムフォレスト","LightGBM","XGBoost","ニューラルネットワーク(ディープラーニング)"]
            st.sidebar.selectbox('学習の手法',activities)
            deep = st.sidebar.number_input('決定木の深さ(サーバの負荷軽減の為 Max=3)',1,3)
